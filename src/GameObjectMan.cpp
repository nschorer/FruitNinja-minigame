...
void GameObjectMan::Terminate()
{
	...
	GameObject* ptmp = (GameObject*)pGOM->pRootTree->getRoot();

	pGOM->privTerminateHelper(pGOM->pRootTree->getRoot());
...
void GameObjectMan::privTerminateHelper( PCSNode* pNode)
{
	// iterate through all of the active children 
	if (pNode->getChild() != 0)
	{
		...
		while (pChild != 0)
		{
			this->privTerminateHelper(pChild);
			// goto next sibling
			GameObject* pTmp = (GameObject*)pChild;
			pChild = pChild->getSibling();
			this->Delete(pTmp);
		}
	}
...
// Remove subtree with root pObj
void GameObjectMan::RemoveSubtree(GameObject* pObj)
{
	GameObjectMan* pGOM = GameObjectMan::privGetInstance();
	assert(pGOM);

	GameObjectMan::AddToDeleteList(pObj);
	pGOM->privRemoveSubtreeHelper(pObj);
}

void GameObjectMan::privRemoveSubtreeHelper(PCSNode* pNode)
{
	// iterate through all of the active children 
	if (pNode->getChild() != 0)
	{
		...
		while (pChild != 0)
		{
			this->privRemoveSubtreeHelper(pChild);
			// goto next sibling
			GameObject* pTmp = (GameObject*)pChild;
			pChild = pChild->getSibling();
			GameObjectMan::AddToDeleteList(pTmp);
		}
	}
}
