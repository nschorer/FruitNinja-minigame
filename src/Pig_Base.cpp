...
Pig_Base::Pig_Base(GameObjectName::Name name, float x, float y, float width, float height, PhysicsFixture* pFixture, float _maxHealth, ImageName::Name _imgNames[2], PhysicsWorld* pWorld)
	:PhysicsObject2D(name, new GraphicsObject_Sprite(_imgNames[0], Rect(x, y, width, height)), pWorld, BodyType::Dynamic, pFixture), imgNames(_imgNames), maxHealth(_maxHealth), health(_maxHealth)
{
}
...
void Pig_Base::CollideVisit(RedBird& other, b2Contact* pcontact, const b2ContactImpulse* pimpulse)
{
	if (pimpulse)
		ContactAction::ContactPair(other, *this, pcontact, pimpulse);
}
